...
export default function LeaveStatusList({ requests }: LeaveStatusListProps) {
  if (requests.length === 0) {
    return (
      <Card>
        <h2 className="text-lg font-bold text-dark-text mb-6">My Leave Requests</h2>
        <EmptyState
          icon={
          ...
          description="You haven't submitted any leave requests yet. Use the form above to request time off."
        />
      </Card>
    );
  }
    ...
    <Card>
      ...
      <div className="space-y-4">
        {requests.map((request) => (
          <div
            key={request.id}
            className="p-4 bg-dark-surfaceHover border border-dark-border rounded-lg hover:border-primary-500/50 transition-colors"
          >
